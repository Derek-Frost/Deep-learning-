...
def check_gradient(f, x, delta=1e-5, tol=1e-4):
    ...
    while not it.finished:
        ix = it.multi_index
        analytic_grad_at_ix = np.mean(analytic_grad)
        numeric_grad_at_ix = 0

        # TODO Copy from previous assignment
        numeric_grad_at_ix = ((f(x+delta)[0]-f(x-delta)[0])/(2*delta))/x.size
        #print(f'numeric_grad_at_ix = {numeric_grad_at_ix}, analytic_grad = {analytic_grad}')
